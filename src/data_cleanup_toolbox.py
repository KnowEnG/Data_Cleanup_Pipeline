...
def run_samples_clustering_pipeline(run_parameters):
    ...
    user_spreadsheet_val_chked = check_input_data_value(user_spreadsheet_df, dropna_colwise=True, check_real_number=True, check_positive_number=True)
    if user_spreadsheet_val_chked is None:
        return False, logging
...
def check_input_value_for_gene_prioritization(user_spreadsheet_df, phenotype_df, correlation_measure):
    ...
    user_spreadsheet_df_dropna = check_input_data_value(user_spreadsheet_df, dropna_colwise=True, check_real_number=True)
